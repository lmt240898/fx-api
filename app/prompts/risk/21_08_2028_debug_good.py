...
def prompt_risk_manager_enhanced(params):
    ...
    - Adds a comprehensive and flat debugging section (STEP 6) with `debug_` prefixed fields to provide full transparency on the AI's calculation path for every decision. This helps pinpoint the exact reason for any outcome, especially failures.
    - All other enhancements from v2.1 are retained.
    """
    ...
    fmessage = f"""
        ...
        **STEP 6: DEBUGGING & TRANSPARENCY OUTPUT (CRITICAL FOR ANALYSIS)**
        - In addition to the main output, you MUST provide the following debugging variables.
        - **All debug fields MUST be prefixed with `debug_` and MUST be at the top level of the JSON. DO NOT use nested objects.**
        - If a calculation is not performed because a preceding step resulted in a `STOP_TRADE`, `SKIP`, or `HOLD` status, the corresponding debug field should be `null`.

        -   `debug_pre_flight_check_status`: The outcome from Step 1 ("CONTINUE", "STOP_TRADE", "SKIP").
        -   `debug_active_order_count`: Integer count of active orders (Step 1a).
        -   `debug_total_potential_risk_pct`: The calculated total unified portfolio risk as a percentage of balance (Step 1d).
        -   `debug_win_prob_risk_pct`: The risk % calculated from the win probability table (Step 3a).
        -   `debug_final_risk_pct`: The final risk % for the trade after capping by `vRisk` (Step 3a).
        -   `debug_risk_in_usd`: The calculated amount to risk in USD (Step 3b).
        -   `debug_stop_loss_in_pips`: The stop loss distance in pips (Step 3b).
        -   `debug_expected_loss_per_lot`: The calculated loss for a 1.0 lot trade (Step 3b).
        -   `debug_base_lot_size`: The initial, unadjusted lot size (Step 3b).
        -   `debug_drawdown_factor`: The multiplier for drawdown control (e.g., 0.7 or 1.0) (Step 3c).
        -   `debug_correlation_factor`: The multiplier for correlation control (e.g., 0.5 or 1.0) (Step 3c).
        -   `debug_effective_positions`: The calculated value for `effective_positions` (Step 3c).
        -   `debug_weighted_position_factor`: The multiplier from `effective_positions` (e.g., 0.5, 0.7, or 1.0) (Step 3c).
        -   `debug_adjusted_lot_before_cap`: The lot size after all adjustments but before the `max_allowed_lot` cap.
        -   `debug_max_allowed_lot`: The calculated max lot size based on account tier (Step 4a).
        -   `debug_lot_after_cap`: The lot size after being capped by `max_allowed_lot`.
        -   `debug_rr_ratio_initial`: The R:R ratio from the proposed signal (Step 4g).
        -   `debug_tp_adjusted_flag`: Boolean (true/false) indicating if the take_profit was adjusted (Step 4g).
        ...
        **RETURN JSON ONLY:**
        ...
        {{
            ...
            "trailing_stop_loss": "integer | null",
            ...
            "debug_pre_flight_check_status": "string | null",
            "debug_active_order_count": "integer | null",
            "debug_total_potential_risk_pct": "float | null",
            "debug_win_prob_risk_pct": "float | null",
            "debug_final_risk_pct": "float | null",
            "debug_risk_in_usd": "float | null",
            "debug_stop_loss_in_pips": "float | null",
            "debug_expected_loss_per_lot": "float | null",
            "debug_base_lot_size": "float | null",
            "debug_drawdown_factor": "float | null",
            "debug_correlation_factor": "float | null",
            "debug_effective_positions": "float | null",
            "debug_weighted_position_factor": "float | null",
            "debug_adjusted_lot_before_cap": "float | null",
            "debug_max_allowed_lot": "float | null",
            "debug_lot_after_cap": "float | null",
            "debug_rr_ratio_initial": "float | null",
            "debug_tp_adjusted_flag": "boolean | null"
        }}
        ```
    """
    return fmessage