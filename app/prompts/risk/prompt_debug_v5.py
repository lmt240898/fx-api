def prompt_risk_manager_enhanced_v5(params):
    """
    Constructs the FINAL, REVISED prompt for the AI Risk and Trade Manager.
    
    VERSION 5.0 REVISIONS:
    - ADDS NEW, ISOLATED EXAMPLES for Step 3c, demonstrating how to apply the "Weighted Position Control" adjustment factor at both the "Light" (factor 0.7) and "Heavy" (factor 0.5) thresholds individually. 
    - This directly targets the inconsistency identified in the v4.0 analysis where the model failed to apply this specific rule correctly when it wasn't part of the main complex example.
    - The original "super-detailed" example for combining multiple factors is preserved for complex scenarios.
    - All other working parts of the prompt, including the JSON output structure with debug fields, are preserved without modification.
    """
