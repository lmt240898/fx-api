...
def prompt_risk_manager_enhanced(params):
    ...
    - CLARIFIED Lot Size Calculation Logic (Step 3b): Added an explicit IF/ELSE IF/ELSE decision tree to force the AI to choose the correct calculation path (A, B, or C).
    - ADDED Concrete Example for Case C (Step 3b): Provided a specific calculation example for a non-USD, non-JPY cross pair (e.g., AUDCAD) to prevent miscalculation.
    - REINFORCED Correlation Control Logic (Step 3c): Rewrote the rule with a more procedural, step-by-step instruction and a clear example to ensure it is always applied correctly.
    - Minor wording adjustments for clarity and enforcement.
    """
    ...
    fmessage = f"""
        ...
        **3b. Calculate Initial Lot Size**:
            *This step calculates the `base_lot_size` before any portfolio adjustments are applied.*
            ...
            2.  **Calculate Stop Loss in Pips/Points (`stop_loss_distance`)**:
                -   `stop_loss_distance = abs(entry_price - stop_loss)`
            3.  **Calculate Expected Loss per Standard Lot (`expected_loss_per_lot`)**:
                -   **MANDATORY DECISION LOGIC**: To choose the correct formula, you MUST identify the quote currency of `{v_symbol}`.
                    - **IF** the quote currency is **USD** (e.g., EURUSD, GBPUSD), you MUST use **Case A**.
                    - **ELSE IF** the quote currency is **JPY** (e.g., USDJPY, EURJPY), you MUST use **Case B**.
                    - **ELSE** (for all OTHER cross pairs like **AUDCAD**, **EURGBP**, **NZDCHF**), you MUST use **Case C**.

                -   **Case A: For Non-JPY pairs (e.g., EURUSD, GBPUSD)** where quote currency is USD:
                    -   `stop_loss_in_pips = stop_loss_distance / symbol_info_json.point`
                ...
                -   **Case B: For JPY pairs (e.g., USDJPY, EURJPY)** where quote currency is JPY:
                    ...
                    -   **Example Calculation**: If SL is 0.40 on USDJPY (40 pips) and `current_usdjpy_rate` is 155.00, then `expected_loss_per_lot` = (40 pips * 1000) / 155.00 = $258.06.

                -   **Case C: For other pairs (e.g., EURGBP, AUDCAD)** where quote currency is NOT USD or JPY:
                    -   `ticks_in_sl = stop_loss_distance / symbol_info_json.trade_tick_size`
                    -   `expected_loss_per_lot = ticks_in_sl * symbol_info_json.trade_tick_value * (1.0 / symbol_info_json.volume_step)`
                    -   **Example Calculation**: For AUDCAD, with SL of 0.0030, `trade_tick_size`=0.00001, `trade_tick_value`=0.7303. `ticks_in_sl` = 0.0030 / 0.00001 = 300 ticks. `expected_loss_per_lot` = 300 * 0.7303 * (1.0 / 0.01) = $219.09.
        ...
        **3c. Apply Portfolio Adjustments**:
            ...
            - **Correlation Control (MANDATORY CHECK)**: You must perform this check.
                 1. Identify the correlation group for `{v_symbol}` from the provided `correlation_groups_json`.
                 2. Count how many ACTIVE orders are ALSO in that same group.
                 3. If this count is 2 or more, you MUST apply a factor of 0.5. `adjusted_lot *= 0.5`.
                 - **Example**: If proposing a BUY on AUDUSD, and there are active BUY orders on EURUSD and GBPUSD, and all three are in the "ANTI_USD_MAJORS" group, the count is 2. You must multiply the lot size by 0.5.
        ...
        **STEP 5: INTERNAL CHECKLIST (MUST VERIFY BEFORE OUTPUT)**
        ...
        - [ ] Did I use the correct `expected_loss_per_lot` formula for {v_symbol} by following the MANDATORY DECISION LOGIC in Step 3b?
        - [ ] Did I correctly check for and apply the Correlation Control factor in Step 3c?
        ...
        **STRICT OUTPUT CONSISTENCY RULES (HARD):**
        ...
        - **IF `lot_size` IS `null`, THE `status` MUST BE `"HOLD"` OR `SKIP` OR `STOP_TRADE`**, and all other execution-related fields must also be `null`.
