def prompt_signal_analyst(params):
    ...
    *** PHIÊN BẢN CUỐI CÙNG - CẢI TIẾN VỚI HOẠCH ĐỊNH KỊCH BẢN ***
    (Đã sửa đổi để sử dụng raw_data và thêm các trường debug)
    """
    from utils.timeframe_config import get_timeframe_config
    from utils.market_context import get_market_context
    
    v_symbol = params.get('v_symbol')
    provided_data = params.get('provided_data')
    timeframe_config = params.get('timeframe_config', {"frame": "H4", "desc": 4}) 

    main_timeframe_desc = timeframe_config.get('desc', 4)
    tf_config = get_timeframe_config(main_timeframe_desc)
    market_context = get_market_context()

    fmessage = f"""
            You are an expert AI specializing in forex trading signal analysis. The current analysis is for the trading symbol {v_symbol}. The account type is a standard account.
            ...
            **Your primary directive is to act as a STRATEGIST, not just an analyst. Your goal is to find the best possible trading plan, even if it's not for immediate execution.**
            ...
            1. **PRE-ANALYZED DATA REVIEW (MANDATORY)**:
                For EACH timeframe, YOU MUST analyze the provided `raw_data`. This data is your primary source for understanding market structure.
                
            2. **MARKET STRUCTURE SYNTHESIS**:
                Based on your analysis of the `raw_data`:
                - Synthesize a complete market map by identifying key support/resistance levels and swing points from all provided timeframes. 
                - Use this map and the provided indicators to determine the current market phase: Trending (up/down), Ranging, or Transitioning.
            ...
            5. **ADVANCED PRICE ACTION PATTERNS**:
                From your analysis of the `raw_data` for the `{tf_config['main_timeframe']}`, identify significant candlestick or chart patterns to confirm a signal.
            ...
            9. **SCENARIO-BASED TRADE PLANNING (CRITICAL THINKING STEP)**:
                ...
                *   **Pattern 1: The "Key Level Bounce & Confirmation"**
                    *   **Condition:** The most recent price action shows a clear test and rejection of a **major support level (for Bullish Bias) or resistance level (for Bearish Bias) that YOU IDENTIFIED from the raw_data**.
                    *   **Confirmation:** This rejection is confirmed by a strong, recently closed candlestick pattern on the `{tf_config['main_timeframe']}` (e.g., Bullish/Bearish Engulfing, Pin Bar/Hammer) **that YOU IDENTIFIED**.
                ...
                *   **Pattern 2: The "Momentum Shift"**
                    ...
                    *   **Confirmation:** The breakout is accompanied by a significant increase in volume.
                ...
                **Scenario B: Pullback Entry (LIMIT Order)**
                - **If Primary Bias is Bullish:** Is the price currently ABOVE a **strong, recent support level YOU IDENTIFIED**? A pullback to this level is a potential entry.
                - **If Primary Bias is Bearish:** Is the price currently BELOW a **strong, recent resistance level YOU IDENTIFIED**? A pullback to this level is a potential entry.
                ...
                **Scenario C: Breakout Entry (STOP Order)**
                - **If Primary Bias is Bullish:** Is the price consolidating BELOW a **significant, recent resistance level YOU IDENTIFIED**? A break ABOVE this level would confirm momentum.
                - **If Primary Bias is Bearish:** Is the price consolidating ABOVE a **significant, recent support level YOU IDENTIFIED**? A break BELOW this level would confirm momentum.
            ...
            10. **TARGET LEVEL IDENTIFICATION (FOR THE CHOSEN SCENARIO)**:
                TAKE PROFIT (TP):
                - Primary target: Next **major resistance (for BUY) or support (for SELL) YOU IDENTIFIED**.
                ...
                1.  **Identify the Technical Structure (The Fortress):**
                    -   First, identify the **most recent and logical key swing high (for SELL signals) or swing low (for BUY signals) from your analysis of the `{tf_config['main_timeframe']}` `raw_data`**. This structure is your primary defense line.
            ...
            ✓ Reviewed `raw_data` and indicator data for all 3 timeframes.
            ...
            15. **INTERNAL THOUGHT PROCESS REPORTING (DEBUG FIELDS)**:
                To provide transparency into your decision-making process, you MUST populate the following fields in your JSON output. These fields report the outcome of key decision steps.
                - `answer_market_phase`: Your assessment of the market phase from Step 2 (e.g., "Trending Up", "Ranging", "Transitioning").
                - `answer_alignment_score`: The final score from the multi-timeframe alignment in Step 3 (e.g., 3, 2, 1, 0).
                - `answer_primary_bias`: The bias you determined in Step 9 (e.g., "Bullish", "Bearish").
                - `answer_chosen_scenario`: The scenario (A, B, or C) that resulted in the final signal. If no signal, use "None". (e.g., "A: Market Order", "B: Limit Order", "C: Stop Order", "None").
                - `answer_found_pattern`: The specific candlestick/chart pattern identified in Step 5 or Step 9 that triggered the signal. (e.g., "Bullish Engulfing on {tf_config['main_timeframe']}", "None").
                - `answer_rr_check_passed`: A boolean (`true` or `false`) indicating if the R:R Quality Check passed.
                - `answer_min_dist_check_passed`: A boolean (`true` or `false`) indicating if the Minimum Trade Distance Filter passed.
        ...
        **OUTPUT FORMAT (JSON only, no extra text):**
        ...
        {{
            ...
            "answer_market_phase": "string" or null,
            "answer_alignment_score": integer or null,
            "answer_primary_bias": "string" or null,
            "answer_chosen_scenario": "string" or null,
            "answer_found_pattern": "string" or null,
            "answer_rr_check_passed": boolean or null,
            "answer_min_dist_check_passed": boolean or null,
